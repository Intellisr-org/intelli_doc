...
class SuryaOCRConfig(PretrainedConfig):
    ...
    def __init__(self, **kwargs):
        ...
        # Handle both cases: when encoder/decoder are passed as kwargs or when they're already set
        ...
        # Set token IDs if decoder config is available
        if hasattr(self, 'decoder') and self.decoder is not None:
            if isinstance(self.decoder, dict):
                self.decoder_start_token_id = self.decoder.get("bos_token_id", 1)
                self.pad_token_id = self.decoder.get("pad_token_id", 0)
                self.eos_token_id = self.decoder.get("eos_token_id", 1)
            else:
                self.decoder_start_token_id = getattr(self.decoder, 'bos_token_id', 1)
                self.pad_token_id = getattr(self.decoder, 'pad_token_id', 0)
                self.eos_token_id = getattr(self.decoder, 'eos_token_id', 1)
        else:
            # Default values
